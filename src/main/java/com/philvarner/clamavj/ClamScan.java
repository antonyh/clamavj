...
public class ClamScan {
	...
	// It is mandatory to prefix this command with n or z, and all commands
	// inside IDSESSION must be
	// prefixed.
	//
	// Start/end a clamd session. Within a session multiple SCAN, INSTREAM,
	// FILDES, VERSION, STATS
	// commands can be sent on the same socket without opening new connections.
	// Replies from clamd
	// will be in the form '<id>: <response>' where <id> is the request number
	// (in ascii, starting
	// from 1) and <response> is the usual clamd reply. The reply lines have
	// same delimiter as the
	// corresponding command had. Clamd will process the commands
	// asynchronously, and reply as soon
	// as it has finished processing.
	//
	// Clamd requires clients to read all the replies it sent, before sending
	// more commands to pre-vent prevent
	// vent send() deadlocks. The recommended way to implement a client that
	// uses IDSESSION is with
	// non-blocking sockets, and a select()/poll() loop: whenever send would
	// block, sleep in
	// select/poll until either you can write more data, or read more replies.
	// Note that using non-blocking nonblocking
	// blocking sockets without the select/poll loop and alternating
	// recv()/send() doesn't comply
	// with clamd's requirements.
	//
	// If clamd detects that a client has deadlocked, it will close the
	// connection. Note that clamd
	// may close an IDSESSION connection too if you don't follow the protocol's
	// requirements.
	...
	/**
	 * The method to call if you already have the content to scan in-memory as a
	 * byte array.
	 * 
	 * @param in
	...
	/**
	 * The preferred method to call. This streams the contents of the
	 * InputStream to clamd, so the entire content is not loaded into memory at
	 * the same time.
	 * 
	 * @param in
	 *            the InputStream to read. The stream is NOT closed by this
	 *            method.
